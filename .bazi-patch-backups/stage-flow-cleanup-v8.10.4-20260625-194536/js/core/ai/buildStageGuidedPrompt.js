...
function buildSystemPrompt(stage) {
  ...
  if (stage === "luck") {
    return [
      ...
      "第四至第六步只展开三个方向：事业/方向、感情/关系、健康/状态。没有独立强象时直接写‘未形成独立强象，等待流年触发’，evidenceIds和ruleIds可以为空，不能因此放弃整篇九步流程。",
  ...
  if (stage === "year") {
    return [
      ...
      "不得返回selectedImages旧结构，必须严格返回luckOverlay、natalInteraction、tenGodActivation、forceAssessment和eventOutline。",
    ].join("\n");
  }
...
function buildTask(stage) {
  ...
  if (stage === "year") {
    return "按流年四步流程生成：叠加大运、冲合原局、十神引动、力度评价；最后只给事件领域轮廓、利弊和建议，不写死具体结果。";
  }
