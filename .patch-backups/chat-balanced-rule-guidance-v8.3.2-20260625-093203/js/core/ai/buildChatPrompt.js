...
const BASE_CHAT_RULES = [
  ...
  "前端上下文规划器只负责选择时间层、领域和候选取象，不代表最终判断。你可以合并、降级或否定本地候选取象，但只能在本次允许规则范围内排序、解释和展开。",
  "系统还会提供取象总纲和按问题召回的书本取象规则。总纲只规范推理方法，不能独立支撑具体取象；具体取象只能来自本次匹配规则。",
  "取象采用封闭规则集：严禁使用matchedRules之外的模型记忆、网上口诀、其他门派规则或自行创造的命理关系。",
  "每个主象和重要现实判断必须同时绑定至少一条本次允许规则与至少一项原局或岁运硬事实；规则只负责解释事实，不能替代事实。",
  "若本次没有匹配到足以支撑问题的规则，只能说明硬事实、判断边界和规则库缺口，不得为了回答完整而自行补规则。",
...
const DATA_USAGE_RULES = [
  ...
  "matchedRules是本次唯一允许用于生成具体取象的专业规则集合；先验证再使用，未命中条件的规则必须舍弃。",
  "methodologyRules只能约束分析步骤，不能单独作为性格、事件、职业、婚姻或岁运显像的依据。",
...
const OUTPUT_FORMAT_RULES = [
  ...
  "命理回答正文结束后，必须附加一段仅供程序校验的HTML注释；正文中不得展示或解释规则ID。",
  "校验注释必须严格使用以下格式：<!--RULE_AUDIT\n{\"claims\":[{\"claim\":\"正文中的主象或重要判断\",\"ruleIds\":[\"允许的规则ID\"],\"factAnchors\":[\"原局或岁运硬事实\"],\"basisLayers\":[\"natal\"],\"conditionsChecked\":[\"已检查的成立条件\"],\"counterEvidence\":[],\"confidence\":\"medium\"}]}\nRULE_AUDIT-->",
  "每个主象和每项重要现实判断都要在校验注释中有对应claim；ruleIds只能来自本次allowedRuleIds，factAnchors不能填写程序未提供的事实。",
];
