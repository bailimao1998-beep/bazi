...
export function buildImageryRulePack({
  ...
  return {
    ...
    instruction: [
      "采用封闭规则集：只能使用matchedRules中的规则生成取象，不得调用模型记忆中的其他命理口诀或门派规则。",
      "总纲只规定推理方法，不能单独支撑具体取象；匹配规则也不是现实事实。",
      "必须逐条检查规则的触发证据、成立条件、削弱条件和禁止越级结论。",
      "硬事实与规则冲突时舍弃规则；多条规则冲突时按直接作用、当前层级、条件完整和来源支持仲裁。",
      "每个主象和重要现实判断必须同时绑定至少一条允许规则和至少一项硬事实。",
...
function buildRuleConstraint(
  ...
  return {
    ...
    fallbackWhenNoRuleMatches:
      "只陈述硬事实、数据缺口和需要扩充的规则范围，不得凭模型记忆自行取象。",
    auditSchema: {
      claims: [
        {
          claim:
