...
export function validateChatResponse({ text = "", prompt = {} } = {}) {
  ...
  if (!response) return { valid: false, violations: ["empty_response"], payload };
  ...
  const age = Number(payload?.subjectContext?.age);
  ...
  const eventSection = extractSection(response, "## 主要显像");
  const eventItems = splitEventItems(eventSection);
  if (eventItems.length > 3) violations.push("too_many_event_possibilities");

  eventItems.forEach((item, index) => {
    if (!/可能性/.test(item)) violations.push(`event_missing_probability:${index + 1}`);
    if (!/依据A/.test(item) || !/依据B/.test(item)) {
      violations.push(`event_missing_two_evidence:${index + 1}`);
    }
    if (!/(成立条件|前提)/.test(item)) violations.push(`event_missing_condition:${index + 1}`);
    if (!/(削弱因素|反证|不显像条件)/.test(item)) {
      violations.push(`event_missing_counterevidence:${index + 1}`);
    }
  });

  const adviceSection = extractSection(response, "## 行动建议");
  if (intent !== "free" && !adviceSection.trim()) violations.push("missing_action_advice");
  if (/包治|转运必成|保证发财|稳赚|替代就医|停止用药/.test(adviceSection)) {
    violations.push("unsafe_advice");
  }

  return { valid: violations.length === 0, violations: unique(violations), payload };
}
...
export function buildChatRepairPrompt({ basePrompt = {}, draft = "", violations = [] } = {}) {
  return {
    ...
    system: [
      ...
      "- 主要显像最多三项，每项包含可能性、依据A、依据B、成立条件和削弱因素。",
      "- 建议必须与主要显像对应，现实可执行、低风险。",
      "- 不输出纠错说明，只输出最终答案。",
    ].join("\n"),
...
function localizeViolation(value) {
  ...
  const map = {
    ...
    too_many_event_possibilities: "主要显像超过三项",
