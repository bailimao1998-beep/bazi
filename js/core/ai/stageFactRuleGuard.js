...
export function normalizePillarTenGod(pillar, dayMaster) {
  ...
  const reportedStemTenGod = text(pillar.stemTenGod);
  const reportedBranchMainTenGod = text(pillar.branchMainTenGod);
  const isDayMasterPillar = isNatalDayPillar(pillar);

  // 日柱天干是命主本人，业务展示应保留“日主”。
  // 理论换算上它与自身同五行同阴阳，对应比肩，但不能因此把“日主”判成错误。
  const normalizedStemTenGod = isDayMasterPillar
    ? (reportedStemTenGod === "比肩" || !reportedStemTenGod ? "日主" : reportedStemTenGod)
    : (expectedStemTenGod || reportedStemTenGod);

  return {
    ...pillar,
    isDayMasterPillar,
    stemRole: isDayMasterPillar ? "day_master" : "ten_god",
    computedStemTenGod: expectedStemTenGod,
    computedBranchMainTenGod: expectedBranchMainTenGod,
    reportedStemTenGod,
    reportedBranchMainTenGod,
    stemTenGod: normalizedStemTenGod,
    branchMainTenGod: expectedBranchMainTenGod || reportedBranchMainTenGod,
...
export function canonicalizeStructureFact(fact = {}) {
  ...
  else if (/(?:^|[_-])harm(?:$|[_-])|branch.*harm|害|穿/.test(type)) relation = "害";
  ...
  return {
    ...fact,
    relation,
    hardFact: CANONICAL_RELATIONS.has(relation) && text(fact.category) !== "hierarchy",
...
function validateTenGodFields({ pillar, dayMaster, label, errors }) {
  ...
  if (expectedStem && reportedStem) {
    const acceptedDayMasterLabels = new Set(["日主", expectedStem]);
    if (isDayMasterPillar) {
      if (!acceptedDayMasterLabels.has(reportedStem)) {
        errors.push(`${label}${stem}为日主，当前错误标记为${reportedStem}`);
      }
    } else if (expectedStem !== reportedStem) {
...
function isNatalDayPillar(pillar = {}) {
  return text(pillar?.key) === "day"
    || text(pillar?.name) === "日柱"
    || pillar?.isDayMasterPillar === true
    || pillar?.stemRole === "day_master";
}
