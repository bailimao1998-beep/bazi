...
const OUTPUT_FORMAT_RULES = [
  ...
  "完成前面所有详细分析后，必须在回答最末尾追加“## 总结”。",
  "总结必须建立在前文全部分析之上，综合回答用户真正关心的整体结论、主要主线、重要时间或成立条件、现实影响和关键建议。",
  "问题涉及多个领域、多个年份或多个阶段时，总结应分别概括重要部分，不得为了刻意简短而遗漏核心结论。",
  "总结不限制固定条数、段落数或字数，以用户不阅读全文也能掌握整体答案为准；但不要重复前文的大量干支、十神、规则和推导过程。",
  "总结不得引入前文没有论证的新判断，也不得与前文结论相互矛盾。",
  "所有Markdown标题必须单独占一行，标题前后应换行；严禁把##标题直接粘在上一句话后面。",
  "项目符号和编号列表必须一项一行，严禁把1、2、3等多个编号连续写在同一个段落中。",
  "AI问答处于窄宽度对话窗口，禁止输出Markdown表格；涉及多年或多阶段比较时，改用按年份或阶段分组的列表，每个年份或阶段单独一项。",
  "总结可以使用自然段或分组列表，结构随问题复杂度决定；总结只概括结论、阶段、时间、风险与建议，不重复大段命理推导。",
  "总结中不要出现规则库、内部校验、候选取象、正式格局判定等面向开发者或推导过程的表述。",
