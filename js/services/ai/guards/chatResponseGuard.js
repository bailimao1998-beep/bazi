...
const STEM_CONTROL_PATTERN =
  /([甲乙丙丁戊己庚辛壬癸])(?:[木火土金水])?\s*(?:克制|克)\s*([甲乙丙丁戊己庚辛壬癸])(?:[木火土金水])?/g;
...
const COMPACT_STEM_COMBINATION_PATTERN =
  /([甲乙丙丁戊己庚辛壬癸])([甲乙丙丁戊己庚辛壬癸])合/g;

const VERBOSE_STEM_COMBINATION_PATTERNS = [
  /([甲乙丙丁戊己庚辛壬癸])(?:[木火土金水])?\s*(?:与|和|同)\s*([甲乙丙丁戊己庚辛壬癸])(?:[木火土金水])?.{0,10}(?:天干五合|五合|相合|合着|合住|合向)/g,
  /([甲乙丙丁戊己庚辛壬癸])(?:[木火土金水])?.{0,8}(?:也)?(?:天干五合|五合|相合|合着|合住|合向).{0,8}([甲乙丙丁戊己庚辛壬癸])(?:[木火土金水])?/g,
];
...
const STEM_TEN_GOD_PATTERN =
  /([甲乙丙丁戊己庚辛壬癸])(?:[木火土金水])?(比肩|劫财|正印|偏印|食神|伤官|正财|偏财|正官|七杀)/g;
...
export function validateChatResponse({ text = "", prompt = {} } = {}) {
  ...
  splitSentences(response).forEach((sentence) => {
    ...
    if (/(旺至极|身极旺|身极强|极弱|从格|假从|专旺)/.test(sentence) && !hasConditionalMarker(sentence)) {
...
export function buildChatRepairPrompt({ basePrompt = {}, draft = "", violations = [] } = {}) {
  return {
    ...
    system: [
      ...
      "",
      "重写要求：",
      "- 交运年按交运前后分段，并按实际覆盖时间确定全年主次；年末换运时旧运加流年仍是全年主背景。",
      "- 天干五行生克方向必须正确；不确定时改写为双方存在作用，不要写反。",
      "- 天干五合只允许甲己、乙庚、丙辛、丁壬、戊癸；例如丙癸不合。",
      "- 十神名称必须与日主和天干的确定关系一致。",
      "- 大运是否结束、是否为最后一年，只以实际交运年月为准，不依据展示年份范围猜测。",
      "- 没有流月基础数据时，删除具体月份、季节或上下半年判断，不要用含糊占位词替换。",
      "- 正式格局、喜用和合化必须写完整成立依据；条件不全时降级为倾向。",
      "- 必须先写核心取象，至少明确主象，再写辅象、矛盾象、条件象和反证象。",
      "- 窄问题的可能表现最多三项；全面问题可以分领域展开，但必须有依据、条件和削弱因素。",
      "- 匹配规则优先；规则不足时可依据硬事实和取象总纲保守补充，但不得自创具名关系、正式格局或特殊口诀。",
      "- 不要在正文中输出英文规则ID或内部字段。",
...
export function sanitizeChatResponse({ text = "", prompt = {} } = {}) {
  ...
  output = sanitizeInvalidStemControls(
    output,
  );

  output = sanitizeInvalidStemCombinations(
    output,
  );

  output = sanitizeExplicitTenGodLabels({
    value:
      output,
    dayMaster:
      payload
  ...
  output = sanitizeInvalidLuckStatusClaims({
    value:
      output,
    payload,
  });
...
function validateStemControlDirections(
  response,
) {
  const violations = [];
  ...
  for (
    ...
    if (
      ...
      violations.push(
        `invalid_stem_control:${controller}克${controlled}`,
      );
    }
  }
...
function sanitizeInvalidStemControls(
  value,
) {
  return String(
    value ??
    ...
    (
      ...
      if (
        ...
        return `${controlled}${STEM_ELEMENT[controlled]}克制${controller}${STEM_ELEMENT[controller]}`;
      }

      return `${controller}与${controlled}之间存在五行作用`;
    },
  );
}
...
function validateStemCombinations(
  response,
) {
  const violations = [];

  collectStemCombinationClaims(
    response,
  ).forEach(
    (
      {
      ...
      if (
        !isCanonicalStemCombination(
          first,
          second,
        )
      ) {
        violations.push(
          `invalid_stem_combination:${first}${second}`,
        );
      }
    },
  );
...
function sanitizeInvalidStemCombinations(
  value,
) {
  return String(
    value ??
...
function collectStemCombinationClaims(
  value,
) {
  const text =
    String(
  ...
  for (
    const matched of
      text.matchAll(
        COMPACT_STEM_COMBINATION_PATTERN,
      )
  ) {
    claims.push({
      first:
  ...
  VERBOSE_STEM_COMBINATION_PATTERNS
    .forEach(
      (pattern) => {
        pattern.lastIndex =
          0;
...
function isCanonicalStemCombination(
  first,
  second,
) {
  return STEM_COMBINATION_PARTNER[
    first
  ] ===
    second;
}

function validateExplicitTenGodLabels({
  response,
  dayMaster,
} = {}) {
  const normalizedDayMaster =
    String(
      dayMaster ??
      "",
    );
  ...
  for (
    ...
    if (
      ...
      violations.push(
        `invalid_ten_god_label:${stem}:${actual}:${expected}`,
      );
    }
  }
...
function sanitizeExplicitTenGodLabels({
  value,
  dayMaster,
} = {}) {
  const normalizedDayMaster =
...
function validateLuckStatusClaims({
  response,
  payload,
} = {}) {
  const timeline =
  ...
  const claimsFinalYear =
    /(?:大运|运程|此运|这步运).{0,12}(?:最后一年|收官之年|终结之年)|(?:最后一年|收官之年|终结之年).{0,12}(?:大运|运程|此运|这步运)/.test(
      text,
    );
  ...
  if (
    ...
    return [
      `invalid_luck_status:${year}:not_final_year`,
    ];
  }
...
function sanitizeInvalidLuckStatusClaims({
  value,
  payload,
} = {}) {
  const timeline =
  ...
  return String(
    ...
    .replace(
      /(?:当前|本年|今年)?\s*(?:正处于|处于)?\s*([^。！？；\n]{0,20}大运)?\s*(?:的)?(?:最后一年|收官之年|终结之年)/g,
      (
        _full,
        luckName,
      ) =>
        `${luckName ?? "当前大运"}仍在运行，尚未到实际交运时间`,
    );
}

function uniqueObjectsByKey(
  items,
  keyBuilder,
) {
  const seen =
...
function hasUnsupportedSpecificTiming(
  response,
  payload,
) {
  const allowed =
    collectAllowedTimingTokens(
      payload,
    );
  ...
  return String(
    ...
    .some(
      (line) =>
        collectTimingTokensForLine(
          line,
        ).some(
          (token) =>
            !allowed.has(
              normalizeTimingToken(
                token,
              ),
            ),
        ),
...
function collectAllowedTimingTokens(
  payload,
) {
  const allowed =
    new Set();
...
function removeUnsupportedTiming(
  response,
  payload,
) {
  const allowed =
  ...
  return String(
    ...
    .filter(
      (line) => {
        ...
        return !collectTimingTokensForLine(
          line,
        ).some(
          (token) =>
            !allowed.has(
...
function collectTimingTokensForLine(
  value,
) {
  const text =
    String(
  ...
  const tokens = [
    ...
    ...(
      text.match(
        /(?<![农历])[一二三四五六七八九十]{1,3}月/g,
      ) ??
      []
    ),
    ...(
      text.match(
        /(?:上半年|下半年|春季|夏季|秋季|冬季)/g,
      ) ??
      []
    ),
  ];
  ...
  if (
    /机会|明显|容易|适合|增强|减弱|出现|发生|推进|注意|高发|时间节奏|月份|哪月|几月/.test(
      text,
    )
  ) {
    tokens.push(
...
function localizeViolation(value) {
  ...
  if (text.startsWith("invalid_stem_control:")) return "天干五行生克方向写反";
  if (text.startsWith("invalid_stem_combination:")) return "使用了不成立的天干五合关系";
  if (text.startsWith("invalid_ten_god_label:")) return "十神名称与日主和天干的确定关系不一致";
  if (text.startsWith("invalid_luck_status:")) return "大运是否结束或最后一年的判断与实际交运时间不一致";
