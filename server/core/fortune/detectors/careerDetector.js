...
export function detectCareerEvent({ triggerChains = [], matchedRules = [] } = {}) {
  ...
  const baseEvidence = evidenceFromChains(picked);
  const ruleEvidence = baseEvidence.length ? getRuleEvidenceBoost(matchedRules, "career") : emptyRuleEvidence();
  const score = 12
  ...
  return createEventCandidate({
    ...
    debug: { direct: direct.length, officerStars: officerStars.length, resourceStars: resourceStars.length, outputStars: outputStars.length, monthPillarHits: monthPillarHits.length, ruleEvidence },
  });
}
