...
export const baziBasicData = {
  heavenlyStems: STEMS,
  earthlyBranches: BRANCHES,
  hiddenStems: HIDDEN_STEMS,
  tenGods: ["比肩", "劫财", "食神", "伤官", "偏财", "正财", "七杀", "正官", "偏印", "正印"],
  nayinTable: NAYIN_BY_PAIR,
  twelveGrowthTable: TWELVE_STAGE_MATRIX,
  kongWangTable: VOID_BRANCHES_BY_DECADE,
  ganZhiRelations: {
    stemFiveCombinations: [["甲", "己"], ["乙", "庚"], ["丙", "辛"], ["丁", "壬"], ["戊", "癸"]],
    branchSixCombinations: [["子", "丑"], ["寅", "亥"], ["卯", "戌"], ["辰", "酉"], ["巳", "申"], ["午", "未"]],
    branchSixClashes: [["子", "午"], ["丑", "未"], ["寅", "申"], ["卯", "酉"], ["辰", "戌"], ["巳", "亥"]],
    branchSixBreaks: [["子", "酉"], ["丑", "辰"], ["寅", "亥"], ["卯", "午"], ["巳", "申"], ["未", "戌"]],
  },
};

export function getTenGod(dayStem, targetStem, datasets = {}) {
  return datasets?.tenGods?.tenGodMatrix?.matrix?.[dayStem]?.[targetStem] ?? deriveTenGod(dayStem, targetStem);
}

export function getHiddenStems(branch, datasets = {}) {
  ...
  return source.map((item, index) => {
    ...
    return {
      stem,
      role: normalizeHiddenStemRole(index, source.length),
      originalRole: item.role ?? "",
      element: item.element ?? getStemElement(stem),
      yinYang: getYinYang(stem),
      weight: item.weight ?? (index === 0 ? 100 : 0),
    };
  });
}
...
export function getYinYang(char) {
...
export function getTwelveGrowth(dayStem, branch, datasets = {}) {
...
export function getKongWang(ganzhi) {
  ...
  const xunStem = STEMS[xunStart % 10];
  const xunBranch = BRANCHES[xunStart % 12];
  return {
    pillar: `${stem}${branch}`,
    xun: `${xunStem}${xunBranch}旬`,
    branches: VOID_BRANCHES_BY_DECADE[Math.floor(index / 10)] ?? [],
  };
}

export function getGanZhiRelations(chart, datasets = {}) {
...
export function getElementStats(chart, mode = "visible", datasets = {}) {
...
export function getBasicBaziDisplay({ input = {}, birth = {}, pillars, datasets = {}, chartMeta = {} }) {
  ...
  return {
    ...
    tenGods: {
      ...
      stats: {
        fullHidden: countBy(PILLAR_KEYS.flatMap((key) => enrichedPillars[key].hiddenStems.map((item) => item.tenGod))),
        mainQi: countBy(PILLAR_KEYS.map((key) => enrichedPillars[key].branchMainTenGod)),
      },
      notes: {
        fullHidden: "按完整藏干统计",
        mainQi: "按地支主气统计",
    ...
    elementStats: {
      visible: getElementStats(pillars, "visible", datasets),
      hidden: getElementStats(pillars, "hidden", datasets),
    },
    nayin: Object.fromEntries(PILLAR_KEYS.map((key) => [key, enrichedPillars[key].nayin])),
    twelveGrowth: {
      note: "十二长生按日主天干推算",
      byPillar: Object.fromEntries(PILLAR_KEYS.map((key) => [key, enrichedPillars[key].twelveGrowth])),
    },
    voids: {
      day: voidsByPillar.day,
      byPillar: voidsByPillar,
    },
    calendar,
    fetalPalaces: {
      note: "当前为近似算法，仅作基础展示，不参与核心判断。",
      fetalOrigin: normalizePalace(chartMeta.fetalOrigin),
      lifePalace: normalizePalace(chartMeta.lifePalace),
      bodyPalace: normalizePalace(chartMeta.bodyPalace),
    },
...
function buildBasicPillar(key, pillar, dayStem, datasets) {
  ...
  const hiddenStems = getHiddenStems(pillar.branch, datasets).map((item) => ({
    ...item,
    tenGod: getTenGod(dayStem, item.stem, datasets),
    elementLabel: formatElementYinYang(item.stem),
  }));
  return {
    key,
...
function makeRelation(type, pillars, members) {
  return {
    type,
    pillars: pillars.map((pillar) => PILLAR_LABELS[pillar.key]),
...
function normalizePalace(palace) {
  ...
  return {
    ...
    method: palace.meta?.method ?? "近似算法",
  };
}
