(function () {
  const DATASET_KEYS = ["blindCases", "strengthModel", "patternsUsefulGods", "blindCoreMethods", "outputTemplates", "caseStudies", "referenceKnowledge"];
  const GROUP_KEYS = ["structure", "tenGods", "branchRelations", "blindCandidates", "strengthUsefulGods", "luckFlow", "cases"];
  ...
  function buildLearningInterpretations(reading = {}, datasets = {}) {
    ...
    return { learningInterpretations, grouped: buildGrouped(learningInterpretations) };
  }
  ...
  function buildContext(reading, datasets) {
    ...
    const dayMasterSeason = strengthSignals.find((item) => item.element === pillars.day?.stemElement);
    return {
      reading,
      datasets,
      pillars,
      ...
      tags: collectReadingTags(reading),
    };
  }
  ...
  function normalizeRuleInterpretation(rule, context) {
    ...
    return createInterpretation({
      ...
      reason,
      learningLogic: rule.logic ?? evidence.howToLearn ?? "把命中条件放回柱位、十神、旺衰和岁运中逐层验证。",
      plainExplanation: rule.plainExplanation ?? "这是一条候选学习信号，需要结合更多条件继续验证，不能单独作为结论。",
      evidence: [`触发规则：${rule.trigger ?? rule.category ?? rule.id}`, reason],
      uncertaintyFactors: normalizeTextList(evidence.uncertaintyFactors).length ? normalizeTextList(evidence.uncertaintyFactors) : defaultUncertaintyFactors(),
  ...
  function buildStrengthInterpretations(context) {
    const items = [];
    for (const item of getExtremeElements(context.elements, "max").slice(0, 2)) {
    ...
    for (const item of getExtremeElements(context.elements, "min").filter((entry) => Number(entry.value) <= 1).slice(0, 2)) {
      items.push(createInterpretation({
  ...
  function buildCaseInterpretations(context) {
    return matchCases(context).slice(0, 4).map((caseItem) => createInterpretation({
      id: `case-${caseItem.id}`,
  ...
  function matchCases(context) {
    return context.caseStudies
      .map((caseItem) => {
        const matchedTags = normalizeTextList(caseItem.tags).filter((tag) => context.tags.some((readingTag) => includesEither(readingTag, tag)));
  ...
  function createInterpretation(input) {
    return { matched: true, sourceRefs: [], confidence: "low", status: "draft", ...input, uncertaintyFactors: normalizeTextList(input.uncertaintyFactors).length ? normalizeTextList(input.uncertaintyFactors) : defaultUncertaintyFactors() };
  }
  ...
  function buildGrouped(items) {
    ...
    for (const item of items) grouped[GROUP_KEYS.includes(item.category) ? item.category : "structure"].push(item);
    return grouped;
  }

  function mapCategory(category, datasetKey) {
    if (category?.includes("blind") || datasetKey === "blindCases" || datasetKey === "blindCoreMethods") return "blindCandidates";
    if (category?.includes("strength") || category?.includes("pattern") || datasetKey === "strengthModel" || datasetKey === "patternsUsefulGods") return "strengthUsefulGods";
    if (category?.includes("case") || datasetKey === "caseStudies") return "cases";
    return "structure";
  }
  ...
  function matchesRelationType(expected, relations) {
    ...
    return relations.some((relation) => [relation.type, relation.effect, relation.relation, relation.title, relation.description].filter(Boolean).some((value) => String(value).includes(expected)));
  }
  ...
  function matchesElementStrength(condition, context) {
    ...
    return items.every((item) => {
      ...
      const value = Number(context.elements[item.element] ?? 0);
      if (item.state === "strong") return value >= 3 || isExtremeElement(context.elements, item.element, "max");
      if (item.state === "weak") return value <= 1 || isExtremeElement(context.elements, item.element, "min");
  ...
  function matchesDayMasterSeason(condition, dayMasterSeason) {
    ...
    return (!condition.status || condition.status === dayMasterSeason.seasonalStatus) && (!condition.element || condition.element === dayMasterSeason.element);
  }

  function matchesCaseTags(expected, tags) {
    return normalizeTextList(expected).some((tag) => tags.some((readingTag) => includesEither(readingTag, tag)));
  }

  function countConditionMatches(condition, counts, targetKey) {
    return (Array.isArray(condition) ? condition : [condition]).every((item) => {
      const key = item?.[targetKey];
      return key ? compareNumber(Number(counts[key] ?? 0), item.operator ?? ">=", Number(item.value ?? 1)) : false;
    });
  }
  ...
  function getExtremeElements(elements, mode) {
    ...
    const target = mode === "max" ? Math.max(...entries.map((item) => item.value)) : Math.min(...entries.map((item) => item.value));
    return entries.filter((item) => item.value === target);
  }
  ...
  function normalizeSourceRefs(sources, sourceIds) {
    return [...normalizeTextList(sources).map((note) => ({ note })), ...normalizeTextList(sourceIds).map((sourceId) => ({ sourceId }))];
  }
  ...
  window.BaziLearningInterpretationEngine = { buildLearningInterpretations };
})();
