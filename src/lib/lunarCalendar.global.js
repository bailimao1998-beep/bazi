(function () {
  ...
  function lunarToSolar({ year, month, day, isLeapMonth = false }) {
    ...
    for (let value = start; value <= end; value += DAY_MS) {
      ...
      if (lunar.year === target.year && lunar.month === target.month && lunar.day === target.day && lunar.isLeapMonth === target.isLeapMonth) {
        return solarDate;
      }
    }
  ...
  function getSupportedLunarRange() {
