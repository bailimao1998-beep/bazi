const { formatLunarDate, lunarToSolar, solarToLunar } = window.BaziCalendar;
const { getBasicBaziDisplay } = window.BaziBasics;
const { adaptBaziBasicChart, matchBaziBasicRules } = window.BaziBasicRules;
const { matchReferenceKnowledge: matchReferenceKnowledgeCards } = window.BaziReferenceKnowledge;
...
function analyzeBirth(input, datasets = {}) {
...
function getTransitYears(centerYear = new Date().getFullYear(), range = 5) {
  const years = [];
...
function getTransitMonths(year) {
...
window.BaziEngine = { analyzeBirth, getTransitYears, getTransitMonths, labels };
