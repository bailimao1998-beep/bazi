(function () {
  ...
  function matchReferenceKnowledge(readingContext, referenceKnowledge, limit = DEFAULT_LIMIT) {
  ...
  function buildMatchContext({
    ...
    return { stems, branches, labels, tenGodNames, elementNames, patternNames, starNames, relationTokens: [...relationTokens] };
  }
  ...
  function matchRelation(values, relationTokens) {
    const hits = normalizeList(values).filter((value) => relationTokens.some((token) => token.includes(value) || value.includes(token)));
  ...
  window.BaziReferenceKnowledge = { matchReferenceKnowledge };
})();
