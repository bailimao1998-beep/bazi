...
import { validateChatResponse } from "../../js/services/ai/guards/chatResponseGuard.js";
...
## 核心取象
主象是交运节点与流年重复共同带来的阶段调整，辅象是规则压力，反证是现实安排可能保持稳定。
...
test("全局提示词包含证据层级、交运分段与建议规则", () => {
  ...
  assert.equal(payload.subjectContext.referenceAge, 19);
  ...
  assert.match(result.system, /建议必须对应前文判断并且现实可执行/);
});
...
test("问答守卫不额外强制主要显像栏目", () => {
  const bad = compliant.replace(/- 依据B：[^\n]+\n/, "").replace(/- 削弱因素：[^\n]+\n/, "");
  const result = validateChatResponse({ text: bad, prompt: basePrompt({ transition: true }) });
  assert.equal(result.violations.some((x) => x.startsWith("event_missing_")), false);
});
