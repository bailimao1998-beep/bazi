...
import {
  ...
  formatLuckDateRange,
} from "../../js/domain/transit/stageSemanticModel.js";

import {
  buildStageFixedReportModel,
} from "../../js/domain/transit/buildStageFixedReportModel.js";

import {
  validateStageNarrative,
} from "../../js/services/ai/transit/stageFixedNarrativeService.js";
...
test("语义校验拒绝时间越级、夫妻宫误判、未支持格局和内部key", () => {
  ...
  assert.equal(result.issues.some((issue) => /英文枚举|内部字段/.test(issue)), false);
  assert.ok(result.issues.some((issue) => /绝对化措辞/.test(issue)));
});
...
test("没有逐年证据时拒绝大运前中后期具体年份", () => {
  ...
  assert.equal(result.usable, false);
  assert.ok(result.issues.some((issue) => /具体年份区间/.test(issue)));
});
