...
import { validateStageAiReport } from "../../js/services/ai/contracts/stageAiReportContract.js";
...
test("大运软缺陷不再阻断八步流程", () => {
  ...
  assert.match(result.structured.directions.relationship.summary, /不是最强主线/);
  assert.match(result.structured.directions.healthState.summary, /生活节奏/);
...
test("两次AI核心引用失败后仍返回八步结构而不是只显示事实", async () => {
