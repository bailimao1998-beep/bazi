...
import { renderLuckFlowReport } from "../../js/ui/transit/luckFlowRenderer.js";
import { renderYearFlowReport } from "../../js/ui/transit/yearMonthFlowRenderer.js";
...
test("Prompt和兼容输出同步使用八步大运", async () => {
  const prompt = await readFile(new URL("../../js/services/ai/transit/buildStageGuidedPrompt.js", import.meta.url), "utf8");
  const contract = await readFile(new URL("../../js/services/ai/contracts/stageAiReportContract.js", import.meta.url), "utf8");
  assert.doesNotMatch(prompt, /九步大运|叠加流年说明/);
  assert.match(prompt, /八步大运/);
  assert.match(prompt, /不得在用户可见内容中直接复述/);
